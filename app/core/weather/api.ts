...
// مفتاح OpenWeatherMap الخاص بك
const WAPI_KEY = '57f6e1e77b40f75935345ab0feb09245';

// تعريف واجهة نقطة البيانات للشبكة
...
// --- خوارزميات الذكاء (Confidence & Weights) ---
...
// =========================================================
// 1. دوال الأنيميشن والبيانات الشبكية (Animation Data)
// =========================================================
...
export async function fetchRadarTimestamps() {
  try {
    ...
    if (!res.ok) throw new Error("Radar Timestamps Failed");
    ...
    // إرجاع كائن فارغ لتجنب توقف التطبيق
    return { past: [], nowcast: [], satellite: [] };
  }
}

// ب) بناء الخط الزمني الموحد (Unified Timeline)
...
// ج) جلب شبكة بيانات زمنية (Grid Time Series) للحرارة والضغط
// هذه الدالة تجلب بيانات 24 ساعة قادمة لكل نقطة في الشبكة
export async function fetchGridTimeSeries(bounds: any) {
  ...
  // دقة الشبكة (8x8) لتحقيق توازن بين الأداء والدقة
  const rows = 8; 
  const cols = 8;
  const lats: number[] = [];
  const lngs: number[] = [];
  ...
  for (let i = 0; i <= rows; i++) {
    for (let j = 0; j <= cols; j++) {
      lats.push(latMin + i * latStep);
      lngs.push(lngMin + j * lngStep);
    }
  }
  ...
  const params = new URLSearchParams({
    latitude: lats.join(','),
    longitude: lngs.join(','),
    hourly: 'temperature_2m,pressure_msl,cloudcover',
    forecast_days: '1',
    timezone: 'auto'
  });
  ...
  try {
    ...
    if (!res.ok) throw new Error("Grid Series Fetch Failed");
    const data = await res.json();
    
    const timeSeries: any[] = [];
    
    if (Array.isArray(data) && data.length > 0 && data[0].hourly) {
        const hoursCount = Math.min(24, data[0].hourly.time.length);
        
        for (let h = 0; h < hoursCount; h++) {
            const gridPoints = data.map((pointData: any) => ({
                lat: pointData.latitude,
                lng: pointData.longitude,
                temp: pointData.hourly.temperature_2m[h],
                pressure: pointData.hourly.pressure_msl[h],
                clouds: pointData.hourly.cloudcover[h]
            }));
            
            timeSeries.push({
                time: data[0].hourly.time[h],
                timestamp: new Date(data[0].hourly.time[h]).getTime() / 1000,
                grid: gridPoints
            });
        }
    }
    ...
    console.error("Grid Series Error", e);
    return [];
  }
}

// د) جلب شبكة بيانات لحظية (Single Frame Grid) - للاستخدام عند عدم التحريك
...
// =========================================================
// 2. مقارنة النماذج الستة (Models Consensus)
// =========================================================
export async function getModelsData(lat: number, lon: number) {
  ...
  try {
    ...
    // دالة مساعدة لاستخراج القيم بأمان
...
export async function getCityNameFromCoords(lat: number, lon: number): Promise<string> {
  try {
    ...
    if (addr) {
      return addr.neighbourhood || addr.suburb || addr.quarter || addr.residential || addr.village || addr.city || addr.town || "موقعك";
    }
    return "موقعك";
  } catch {
    return "موقعك";

...
export async function getWeather(lat: number, lon: number, cityName: string): Promise<WeatherData> {
  try {
    ...
    return {
      ...
      windGusts: c.wind_gusts_10m, // <--- الآن تعمل لأنها معرفة في types
    ...
    return {
      ...
      sunrise: "-", sunset: "-", uvIndex: 0, visibility: 10000, cloudCover: 0, dewPoint: 0, rainProb: 0, rainAmount: 0, snowDepth: 0, soilMoisture: 0, evapotranspiration: 0, moonPhase: "",

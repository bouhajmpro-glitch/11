...
import { BarChart3, ChevronUp, ChevronDown, AlertTriangle, Brain, Thermometer, Droplets, Wind, Check, Loader2 } from 'lucide-react'; // <-- تمت إضافة Loader2
import { analyzeWeatherModels, AnalysisResult } from '../analysis';
import { getLocationByIP } from '../core/weather/api';

const WeatherMap = dynamic(() => import('../Map'), { ssr: false, loading: () => <div className="h-full w-full bg-slate-900 animate-pulse flex items-center justify-center text-slate-500">جاري تحميل الرادار...</div> });

// --- مكون الرسم البياني اليدوي (SVG Chart) ---
  ...
  return (
    <div className="h-32 w-full mt-4 bg-slate-800/50 rounded-xl relative overflow-hidden border border-white/5">
      <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
        ...
        <path d={`M0,100 L0,${100 - ((data[0].temp - min)/range)*80 - 10} ${points} L100,${100 - ((data[data.length-1].temp - min)/range)*80 - 10} L100,100 Z`} fill="url(#grad)" />
  ...
  return (
    <div className={`absolute bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-xl border-t border-white/10 transition-all duration-500 z-20 flex flex-col ${isPanelOpen ? 'h-[60vh]' : 'h-12'}`}>
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        <div className="flex justify-between items-center mb-4"><h2 className="text-white font-bold flex items-center gap-2"><Brain className="w-5 h-5 text-purple-400"/> الذكاء التحليلي</h2><div className="text-right"><span className="text-xs text-slate-400 block">دقة النظام</span><span className="text-xl font-black text-green-400">{Math.round(analysis.consensusScore)}%</span></div></div>
        ...
        <table className="w-full text-xs text-right text-slate-300">
          ...
          <tbody>{analysis.allModels.map((m, i) => (<tr key={i} className="border-b border-white/5 hover:bg-white/5"><td className="py-3 flex gap-2 items-center">{m.country} {m.name} {i===0 && <Check className="w-3 h-3 text-green-500"/>}</td><td className="py-3 text-amber-400 font-mono">{m.temp.toFixed(1)}°</td><td className="py-3 text-blue-400 font-mono">{m.rain > 0 ? m.rain : '-'}</td><td className="py-3 font-mono">{m.wind.toFixed(0)}</td></tr>))}</tbody>
        </table>
      </div>
    </div>
  );
...
export default function RadarPage() {
  ...
  useEffect(() => {
    if (navigator.geolocation) { navigator.geolocation.getCurrentPosition((pos) => setCoords({ lat: pos.coords.latitude, lon: pos.coords.longitude }), async () => { const ip = await getLocationByIP(); if (ip) setCoords(ip); else setCoords({ lat: 33.5731, lon: -7.5898 }); }); } else { setCoords({ lat: 33.5731, lon: -7.5898 }); }
  }, []);
